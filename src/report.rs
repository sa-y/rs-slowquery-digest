...
struct Row {
    ...
    #[tabled(rename = "Count")]
    count: u64,
    #[tabled(rename = "Total Time(sec)")]
    total_time: String,
    #[tabled(rename = "Mean Time(sec)")]
    mean_time: String,
...
pub fn print_report(
    ...
    match format {
            ...
            let rows: Vec<Row> = items
                ...
                .map(|item| {
                    ...
                    Row {
                        rank: item.rank,
                        count: item.count,
                        total_time: format!("{:.3}", item.total_time),
                        mean_time: format!("{:.3}", item.mean_time),
        ...
        OutputFormat::Markdown => {
            print_markdown(&items, &mut writer)?;
        }
    }
    Ok(())
}
    ...
    stats_vec
        ...
        .map(|(i, (fp, mut stat))| {
            ...
            let mean = if stat.count > 0 {
                stat.total_time / stat.count as f64
            } else {
                0.0
            };
            let mean_lock_time = if stat.count > 0 {
                stat.total_lock_time / stat.count as f64
            } else {
                0.0
            };
            ...
            let tz_offset = match timezone_str.parse::<chrono::FixedOffset>() {
                ...
                Err(_) => {
                    eprintln!(
                        "Warning: Invalid timezone offset '{}', using UTC.",
                        timezone_str
                    );
...
fn print_html(items: &[ReportItem], writer: &mut dyn Write) -> anyhow::Result<()> {
    ...
    writeln!(
        writer,
        ".copy-btn {{ margin-bottom: 5px; padding: 5px 10px; cursor: pointer; }}"
    )?;
    writeln!(writer, "</style>")?;
    ...
    writeln!(writer, "<tr><th>Rank</th><th>Count</th><th>Total Time(sec)</th><th>Mean Time(sec)</th><th>Query ID</th><th>Query</th></tr>")?;
    writeln!(writer, "</thead>")?;
    ...
    for item in items {
        ...
        writeln!(writer, "<td>{:.3}</td>", item.total_time)?;
        writeln!(writer, "<td>{:.3}</td>", item.mean_time)?;
        writeln!(
            writer,
            "<td class=\"query-id\"><a href=\"#{}\">{}</a></td>",
            item.query_id, item.query_id
        )?;
    ...
    for item in items {
        writeln!(
            writer,
            "<div id=\"{}\" class=\"query-block\">",
            item.query_id
        )?;
        writeln!(
            writer,
            "<h3>Rank {}: Query ID {}</h3>",
            item.rank, item.query_id
        )?;
        writeln!(
            writer,
        ...
        writeln!(writer, "<li>Total Time(sec): {:.3}</li>", item.total_time)?;
        writeln!(writer, "<li>Mean Time(sec): {:.3}</li>", item.mean_time)?;
        writeln!(writer, "<li>P95(sec): {:.3}</li>", item.p95)?;
        writeln!(writer, "<li>P99(sec): {:.3}</li>", item.p99)?;
        writeln!(
            writer,
            "<li>Total Lock Time(sec): {:.3}</li>",
            item.total_lock_time
        )?;
        writeln!(
            writer,
            "<li>Mean Lock Time(sec): {:.3}</li>",
            item.mean_lock_time
        )?;
        writeln!(writer, "</ul>")?;
        ...
        writeln!(
            writer,
            "<div class=\"query-sql\"><pre id=\"norm-sql-{}\">{}</pre></div>",
            item.query_id,
            html_escape(item.normalized_query.trim())
        )?;

        writeln!(writer, "<h4>Worst Case Example</h4>")?;
        writeln!(
            writer,
            "<button class=\"copy-btn\" onclick=\"copyToClipboard('sql-{}')\">Copy SQL</button>",
            item.query_id
        )?;
        writeln!(
            writer,
            "<div class=\"query-sql\"><pre id=\"sql-{}\">{}</pre></div>",
            item.query_id,
            html_escape(item.worst_example_query.trim())
        )?;
...
fn print_markdown(items: &[ReportItem], writer: &mut dyn Write) -> anyhow::Result<()> {
    writeln!(writer, "# Slow Query Digest Report")?;

    writeln!(writer, "## Summary\n")?;
    writeln!(writer)?;
    writeln!(
        writer,
        "|Rank|Count|Total Time(sec)|Mean Time(sec)|Query ID|Query|"
    )?;
    writeln!(writer, "|---:|---:|---:|---:|:---|:---|")?;
    for item in items {
        let mut query_display = format_query(&item.example_query, &OutputFormat::Html);
    ...
    writeln!(writer, "## Detailed Report")?;
    writeln!(writer)?;
    for item in items {
        writeln!(writer, "### {}", item.query_id)?;
        writeln!(writer, "- Rank {}", item.rank)?;
        writeln!(writer, "- Time Range: {}", item.time_range)?;
        writeln!(writer)?;
        writeln!(writer, "### Execution Stats")?;
